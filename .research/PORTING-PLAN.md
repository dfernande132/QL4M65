...
## 0. Estado actual del proyecto (actualizado: 2026-08-23 — **Milestone 2 cerrado. Milestone 3 en marcha: `M3001` (RAM principal 128k migrada de BRAM a HyperRAM) falló en hardware real por un margen de timing al límite; diagnosticado y arreglado en `M3002`, pendiente de confirmación en hardware.**)

**Milestone 3 arrancado (2026-08-23).** Plan por escrito en `.research/milestone3-memory-speed-plan.md` (revisado y aprobado por el usuario antes de tocar código, incluida la decisión de bajar el tope de RAM de 4096k a 2048k para no chocar con la reserva de HyperRAM de los microdrives). Fase 1 (128k de RAM principal, BRAM→HyperRAM, mismo tamaño): diseño de `qram_avm.vhd` + simulación aislada (encontró y arregló un bug real de protocolo antes de compilar) + cableado real en `main.vhd`/`mega65.vhd`/`globals.vhd`/`CORE-R6.xpr` + pantalla de bienvenida actualizada. `M3001` compiló limpio (`WNS=+0.090 ns`, `WHS=+0.011 ns`) pero **falló en hardware real**: arranca, lee mdv1, a veces guarda, pero se cuelga al poco - una vez incluso con el menú OSD abierto, sin responder. Diagnóstico (simulación de contención de 3 maestros reales durante 1,5M ciclos sin reproducir el cuelgue, descartando un bug de protocolo/árbitro; revisión del controlador HyperRAM físico real): la causa real es el dominio de reloj `hr_rwds` (captura de datos de lectura, técnica *source-synchronous* del propio protocolo HyperRAM), con `WHS=+0.011 ns` - el margen de *hold* más ajustado del diseño desde `M2025`, nunca antes puesto a prueba porque nada había generado tráfico denso hacia HyperRAM hasta `qram_avm`. **`M3002` (mismo RTL, estrategia de implementación distinta - `Performance_ExplorePostRoutePhysOpt`, fijada como nuevo valor por defecto) sube ese margen a `WHS=+0.213 ns`** (~19×), confirmado con datos reales del propio informe de *timing* routeado. `.cor` empaquetado. Detalle completo en `DECISIONES.md`, secciones "Milestone 3 — arranque y Fase 1" y "Milestone 3, Fase 1 — M3001 falla en hardware real". **Pendiente de confirmación en hardware real.**

...
**Pendiente ahora:**
- Prueba en hardware real de `QL4M65-CoreQL-M1006_r6.cor` (arreglo del `ipl`/
  `vsync_irq` en `zx8302.v` + `OPTM_DX` más estrecho - ver tabla de pruebas
  abajo y `DECISIONES.md`).
...
| `M1006` | Arreglo del cuelgue: `zx8302.v` `ipl` cambiado de `AND`+"nada pendiente" a `OR`+"algo pendiente" (deja pasar `vsync_irq` propio del zx8302 al margen del IPC externo) + `OPTM_DX` 23→18 en `config.vhd` (menú más estrecho) | *(pendiente de probar)* |
